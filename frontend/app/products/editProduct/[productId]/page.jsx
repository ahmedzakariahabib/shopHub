...
const EditProductForm = ({ product }) => {
  ...
  const { updateProduct, loading, currentProduct, fetchProduct } =
    useProductStore();
  ...
  useEffect(() => {
    if (currentProduct) {
      ...
      if (currentProduct.imgCover) {
        setImgCoverPreview(currentProduct.imgCover);
      }

      // Set existing additional images preview
      if (currentProduct.images && currentProduct.images.length > 0) {
        setImagesPreview(currentProduct.images);
      }
    }
  }, [currentProduct]);
  ...
  useEffect(() => {
    if (formData.category) {
      ...
      if (
        ...
        !filtered.find((sub) => sub._id === formData.subcategory)
      ) {
